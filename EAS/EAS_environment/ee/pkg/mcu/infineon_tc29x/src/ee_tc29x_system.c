...
#include "mcu/infineon_tc29x/inc/ee_tc29x_endinit.h"

/* ENDINIT and SAFETY ENDINIT WATCHDOG Support */
void EE_tc_endint_disable( void ) { 
  EE_tc29x_endinit_set(EE_TC_ENDINIT_DISABLE);
}

void EE_tc_endint_enable( void ) { 
  EE_tc29x_endinit_set(EE_TC_ENDINIT_ENABLE);
}

void EE_tc_safety_endinit_disable( void ) { 
  EE_tc29x_safety_endinit_set(EE_TC_ENDINIT_DISABLE);
}

void EE_tc_safety_endinit_enable( void ) { 
  EE_tc29x_safety_endinit_set(EE_TC_ENDINIT_ENABLE);
}
...
static void EE_tc29x_stm_set_sr0(EE_UINT32 usec, EE_TYPEISR2PRIO intvec);
static void EE_tc29x_stm_set_sr0_next_match(EE_UINT32 usec);
#else
...
static void EE_tc29x_stm_set_sr1(EE_UINT32 usec, EE_TYPEISR2PRIO intvec);
static void EE_tc29x_stm_set_sr1_next_match(EE_UINT32 usec);
#else
...
#if (EE_SYSTEM_TIMER_DEVICE == EE_TC_STM_SR0)
  EE_tc29x_stm_set_sr0_next_match(OSTICKDURATION / 1000U);
#elif (EE_SYSTEM_TIMER_DEVICE == EE_TC_STM_SR1)
  EE_tc29x_stm_set_sr1_next_match(OSTICKDURATION / 1000U);
...
#if (EE_SYSTEM_TIMER_DEVICE == EE_TC_STM_SR0)
  EE_tc29x_stm_set_sr0(OSTICKDURATION / 1000U,
    (EE_TYPEISR2PRIO)EE_ISR2_ID_EE_tc_system_timer_handler);
#elif (EE_SYSTEM_TIMER_DEVICE == EE_TC_STM_SR1)
  EE_tc29x_stm_set_sr1(OSTICKDURATION / 1000U,
    (EE_TYPEISR2PRIO)EE_ISR2_ID_EE_tc_system_timer_handler);
#endif
}
...
void EE_tc29x_configure_clock(EE_UREG fpll) {
  EE_tc29x_configure_clock_internal(fpll);
}
#endif /* EE_MASTER_CPU */
...
EE_UREG EE_tc29x_get_clock( void ) {
  ...
  if ( SCU_PLLSTAT.B.VCOBYST )
  {
    ...
    if ( SCU_PLLSTAT.B.FINDIS )
    {
      ...
      fpll = n * (EE_TC29X_BOARD_FOSC / (p * k2));
    }
  }
  return fpll;
}
...
static EE_UINT32 EE_tc29x_stm_freq_khz;

static EE_UINT32 EE_tc29x_stm_us_ticks ( EE_UINT32 usec ) {
  EE_UINT32 ticks;
  if ( EE_tc29x_stm_freq_khz >= EE_KILO ) {
    ticks = usec * ( EE_tc29x_stm_freq_khz / EE_KILO );
  } else if ( usec >= EE_KILO ) {
    ticks = ( usec / EE_KILO ) * EE_tc29x_stm_freq_khz;
  } else {
    ticks = ( usec * EE_tc29x_stm_freq_khz ) / EE_KILO;
  }
  return ticks;
...
EE_STM_SR0_STORAGE void EE_tc29x_stm_set_sr0(EE_UINT32 usec,
    EE_TYPEISR2PRIO intvec)
{
  ...
  compare_value = EE_tc29x_stm_us_ticks(usec) +
    EE_tc29x_stm_get_time_lower_word();
  ...
  if(intvec != (EE_TYPEISR2PRIO)EE_ISR_UNMASKED) {
     ...
     */
    EE_STM_SR0.U =  EE_TC29X_SRN_TYPE_OF_SERVICE(EE_CURRENTCPU) |
      EE_TC29X_SRN_ENABLE | EE_TC29X_SRN_PRIORITY(intvec);
  } else {
...
EE_STM_SR0_STORAGE void EE_tc29x_stm_set_sr0_next_match(EE_UINT32 usec)
{
  EE_UREG compare_value;
...
EE_STM_SR1_STORAGE void EE_tc29x_stm_set_sr1(EE_UINT32 usec,
  EE_TYPEISR2PRIO intvec)
{
  EE_UREG compare_value;
  ...
  if(intvec != (EE_TYPEISR2PRIO)EE_ISR_UNMASKED) {
     ...
     *  [11..12] Type Of Service (means which CPU will handle it)
     */
    EE_STM_SR1.U = EE_TC29X_SRN_TYPE_OF_SERVICE(EE_CURRENTCPU) |
      EE_TC29X_SRN_ENABLE | EE_TC29X_SRN_PRIORITY(intvec);
  } else {
    /* Disable STM Service Request Source */
...
EE_STM_SR1_STORAGE void EE_tc29x_stm_set_sr1_next_match(EE_UINT32 usec)
{
  EE_UREG compare_value;
...
{
  /* Read Start Point */
  EE_UREG const start = EE_tc29x_stm_get_time_lower_word();
  /* Evaluate End Point */
  EE_UREG const ticks = EE_tc29x_stm_us_ticks(usec);

  while ( ticks > EE_tc29x_stm_get_time_lower_word() - start ) {
    ; /* Wait */
  }
}
