...
#include "mcu/infineon_tc29x/inc/ee_tc29x_internal.h"
/* ENDINIT Support */
...
#define EE_tc29x_cinit _c_init
/* C initialization function declaration */
extern void EE_tc29x_cinit( void );

/* Utility macros to set registers: */

/* Two pre-processor steps to let macros explode */
#define EE_tc29x_extru2(src,start,size) \
  ({ int res; __asm volatile (" extr.u\t %0,%1,%2,%3" : "=d" (res) : "d" (src),\
  "i" (start),"i" (size) : "memory"); res; })
#define EE_tc29x_extru(src,start,size) EE_tc29x_extru2(src,start,size)

#define EE_tc29x_setareg2(areg,val) \
  __asm volatile ( "mov.aa " #areg ",%0"::"a"(&val) : "memory" )
#define EE_tc29x_setareg(areg,val) EE_tc29x_setareg2(areg,val)
...
#define EE_tc29x_cinit() ((void)0)
#else /* EE_EXECUTE_FROM_RAM */
static void EE_tc29x_cinit( void );
...
asm volatile int EE_tc29x_extru (int src, int start, int size)
{
...
__INLINE__ void __ALWAYS_INLINE__ EE_tc29x_csa_init( void );
...
/***************** Configure PLL *****************/
  ...
  /* The internal version of EE_tc29x_configure_clock is an inline
     ...
     (thing done later) */
  EE_tc29x_configure_clock_internal(EE_CPU_CLOCK);
   ...
   */
  EE_tc29x_setareg(a0, EE_SMALL_DATA);

  EE_tc29x_setareg(a1, EE_LITERAL_DATA);
#ifdef __TASKING__
  ...
  EE_tc29x_setareg(a8, EE_A8_DATA);

  EE_tc29x_setareg(a9, EE_A9_DATA);
#endif /* __TASKING__ */
   ...
   */
  EE_tc29x_cinit();      /* initialize data */
