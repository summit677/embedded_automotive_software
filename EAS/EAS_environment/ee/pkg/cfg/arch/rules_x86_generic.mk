...
X86_LINKER_SCRIPT = $(EEBASE)/pkg/mcu/x86/cfg/linker.ld
...
TARGET = $(APP_OUTPUT_ELF).dis

APP_ISO = (APP_OUTPUT_ELF).iso 
...
$(TARGET): $(APP_OUTPUT_ELF) $(APP_ISO)
	$(QUIET) $(EE_OBJDUMP) -CD $< > $@.tmp  && mv $@.tmp $@
...
$(APP_OUTPUT_ELF): $(OBJS) $(LD_FILE) $(LIBDEP)
	$(QUIET) $(EE_LINK) -Wl,-T $(X86_LINKER_SCRIPT) $(CPU_CONFIG) -o $@ \
...
$(APP_ISO): $(APP_OUTPUT_ELF)
	@echo "Building Iso"
	mkdir -p grub_iso/boot/grub
	cp $(APP_OUTPUT_ELF) grub_iso/boot/erika.elf
	cp $(EEBASE)/pkg/mcu/x86/cfg/grub.cfg grub_iso/boot/grub
	grub-mkrescue -o erika.iso grub_iso  > /dev/null 2>&1
##
## Directories
##
...
qemu: $(APP_ISO)
	echo "Running QEMU"
	#$(QEMU)  -cdrom ${APP_ISO} -no-reboot  -machine pc,accel=tcg
